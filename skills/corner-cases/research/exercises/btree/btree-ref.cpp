...
static std::pair<bool, size_t> find_or_index(const std::shared_ptr<node>& n, int key) {
    ...
    if (it != n->keys.end() && *it == key) return {true, 0};
...
bool search(const std::shared_ptr<node>& root, int key) {
    ...
    while (cur) {
        ...
        cur = cur->children[find_child_index(cur, key)];
    }
    return false;
}
...
// Insert: TDD — starts empty, built up via red-green-refactor
void insert(std::shared_ptr<node>& root, int key) {
    ...
    while (!is_leaf(cur)) {
        ...
        if (is_full(cur->children[i])) {
            // Try rotation to make room
            if (i > 0 && !is_full(cur->children[i - 1])) {
                rotate_right(cur, i);
            } else if (i + 1 < cur->children.size() && !is_full(cur->children[i + 1])) {
                rotate_left(cur, i);
            }
            // Recompute after rotation (or no-op if siblings were full)
            std::tie(found, i) = find_or_index(cur, key);
            if (found) return;
            // Split if child is still full (no rotation possible, or key shifted)
            if (is_full(cur->children[i])) {
                split_child(cur, i);
                std::tie(found, i) = find_or_index(cur, key);
                if (found) return;
...
static bool check_node(const std::shared_ptr<node>& n, bool is_root,
                        int lo, int hi, size_t expected_depth, size_t depth) {
    if (!n) return false;

    // Key count bounds
    // Minimum applies to internal non-root nodes only; leaves and root can have fewer
    size_t min_keys = (!is_root && !is_leaf(n)) ? K / 2 : 0;
    ...
    if (is_leaf(n)) {
        // Leaf must be at expected depth
    ...
    for (size_t i = 0; i < n->children.size(); i++) {
        int child_lo = (i == 0) ? lo : n->keys[i - 1];
        int child_hi = (i == n->keys.size()) ? hi : n->keys[i];
        if (!check_node(n->children[i], false, child_lo, child_hi,
...
bool check_invariants(const std::shared_ptr<node>& root) {
    ...
    return check_node(root, true, INT_MIN, INT_MAX, leaf_depth(root), 0);
}
